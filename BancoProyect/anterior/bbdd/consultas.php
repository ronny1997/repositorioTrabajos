...
//grabar movimiento
    ...
    public function ingreso($saldoCuenta, $saldoCliente1, $saldoCliente2, $movimiento) {
...
//saldo clientes
            $this->db->exec($saldoCliente1);

            $this->db->exec($saldoCliente2);
...
function setCuentaSaldo($numCuen, $saldo) {
    $queri = 'select cu_sal from cuentas where cu_ncu = ' . $numCuen;
    $saldoactual = $GLOBALS['conexion']->dato($queri);

    $saldoactual = $saldoactual + $saldo;
    $setSaldoCliente = 'UPDATE cuentas SET cu_sal = ' . $saldoactual . ' WHERE cuentas.cu_ncu = ' . $numCuen ;
    return $setSaldoCliente;
}

function clientesDeCuenta($numCuen) {
    $sql = "select cu_dn1, cu_dn2 from cuentas where cu_ncu =" . $numCuen;
    return $GLOBALS['conexion']->arrayDatos($sql);
}
